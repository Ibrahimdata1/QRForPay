...
// State machine: menu → cart → confirm → paying → status
//
...
export default function CustomerOrderScreen() {
  ...
  const placeOrder = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('เกิดข้อผิดพลาด', err?.message ?? 'สั่งอาหารไม่ได้ กรุณาลองใหม่');
      setScreen('confirm');
    }
  ...
  const subscribeToOrder = (oid: string) => {
    ...
    // Subscribe to order status changes (kitchen workflow)
  ...
  // When payment goes through, move to status screen
  useEffect(() => {
  ...
  const renderHeader = () => (
    <View style={styles.header}>
      <View>
        <Text style={styles.shopName}>{shopName || 'เมนู'}</Text>
        {tableNumber ? (
    ...
    return (
      <View style={styles.container}>
        ...
        >
          ...
          >
            <Text style={styles.statusCircleIcon}>{statusIcon}</Text>
          </View>
                ...
                return (
                  <View key={step} style={styles.stepItem}>
                    <View
                      style={[
                        styles.stepDot,
                        passed && styles.stepDotActive,
                        isCurrent && { borderColor: statusColor, backgroundColor: statusColor + '22' },
                      ]}
                    >
                      {passed ? (
                    ...
                    {i < steps.length - 1 ? (
                      <View
                        style={[
                          styles.stepLine,
                          passed && i < stepIndex && styles.stepLineActive,
                        ]}
                      />
                    ) : null}
                  </View>
          ...
          <View style={styles.confirmCard}>
            <View style={styles.confirmRow}>
              <Text style={styles.summaryLabel}>เลขออเดอร์</Text>
            ...
            <View style={[styles.confirmRow, styles.summaryTotal]}>
          ...
          {isDone ? (
            <View style={styles.doneBox}>
              <Text style={styles.doneText}>ขอบคุณที่ใช้บริการ!</Text>
              <TouchableOpacity
                style={[styles.primaryButton, { marginTop: 16 }]}
                onPress={() => {
                  // Reset for new order
              ...
              >
                <Text style={styles.primaryButtonText}>สั่งอาหารอีกครั้ง</Text>
              </TouchableOpacity>
            </View>
          ) : !isCancelled ? (
