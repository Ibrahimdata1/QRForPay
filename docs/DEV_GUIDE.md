...
│   │   ├── orders.tsx            ← ออเดอร์ (Tab Switcher: กำลังดำเนินการ / ประวัติ)
...
### ออเดอร์ — Tab Switcher (orders.tsx)

หน้าออเดอร์ใช้ **Tab Switcher** สลับระหว่าง 2 แท็บ: `activeTab: 'active' | 'history'`

**แท็บ "กำลังดำเนินการ" (active):**
...
- ใช้ `orderStore.fetchOrderHistory(shopId, dateRange, statusFilter, limit, offset)`
- State: `historyOrders`, `historyTotal`, `historyLoading` ใน orderStore
- **ตัวกรองช่วงเวลา**: วันนี้ / เมื่อวาน / 7 วัน / 30 วัน (pills)
- **ตัวกรองสถานะ**: ทั้งหมด / สำเร็จ / ยกเลิก (pills)
- **สรุปยอด**: bar ด้านบนแสดง ยอดขาย / เงินสด / QR / ยกเลิก
- **ป้ายยืนยัน (confirmation badge)**: แสดงต่อ order card
  - สีส้ม "ยืนยันโดย [ชื่อ]" → `confirmation_type='manual'`, join `confirmed_by` → `profiles.full_name`
  - สี teal "Auto" → `confirmation_type='auto'`
  - สีแดง "ยกเลิกโดย [ชื่อ]" → `cancelled_by` → `profiles.full_name`
- **Pagination**: offset-based, กด "โหลดเพิ่ม" → `offset += limit`, append ผลลัพธ์
- Query ใช้ `{ count: 'exact' }` + `.range(offset, offset + limit - 1)` + order by `completed_at desc nulls last, created_at desc`
